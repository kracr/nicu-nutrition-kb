...
public class ProbabilityPlotiNICU{
        ...
        int numberOfDataPoints = 0;                     // number of data points
        ...
        double[] exponentialOrderMedians = null;        // Exponential order statistic medians
        double[] exponentialParam = null;               // Exponential parameters obtained by the minimization procedure
        ...
        double[] exponentialLine = null;                // Exponential probability plot gradient and intercept
        private double[] exponentialLineErrors = null;          // estimated errors of the Exponential probability plot gradient and intercept
        double exponentialCorrCoeff = Double.NaN;       // Exponential correlation coefficient of the probability plot
        private boolean exponentialDone = false;                // = true after Exponential probability plot drawn

        double[] logNormalParam = null;               // Log Normal parameters obtained by the minimization procedure
        private double[] logNormalParamErrors = null;         // estimates of the errors of the Log Normal parameters obtained by the minimization procedure
        double[] logNormalOrderMedians = null;        // Log Normal order statistic medians
        private double[] logNormalErrors = null;         	  // estimates of the errors of the Log Normal parameters obtained by the minimization procedure
        private double logNormalSumOfSquares = Double.NaN;    // sum of squares at Log Normal minimum
        double[] logNormalLine = null;                // Log Normal probability plot gradient and intercept
        private double[] logNormalLineErrors = null;          // estimated errors of the Log Normal probability plot gradient and intercept
        double logNormalCorrCoeff = Double.NaN;       // Log Normal correlation coefficient of the probability plot
        private boolean logNormalDone = false;                // = true after Log Normal probability plot drawn
        ...
        double[] weibullOrderMedians = null;            // Three Parameter Weibull order statistic medians
        double[] weibullParam = null;                   // Three Parameter Weibull parameters obtained by the minimization procedure
        ...
        double[] weibullLine = null;                    // Three Parameter Weibull probability plot gradient and intercept
        ...
        double weibullCorrCoeff = Double.NaN;           // Three Parameter Weibull correlation coefficient of the probability plot
        ...
        // CONSTRUCTORS
        public ProbabilityPlotiNICU(double[] xx){
        ...
        public ProbabilityPlotiNICU(float[] xx){
             this.arrayAsStat = new Stat(xx);
             this.array = this.arrayAsStat.array();
             this.initialize();
        }
        ...
        public ProbabilityPlotiNICU(long[] xx){
             this.arrayAsStat = new Stat(xx);
             this.array = this.arrayAsStat.array();
             this.initialize();
        }
        ...
        public ProbabilityPlotiNICU(int[] xx){
             this.arrayAsStat = new Stat(xx);
             this.array = this.arrayAsStat.array();
             this.initialize();
        }

        public ProbabilityPlotiNICU(Integer[] xx){
             this.arrayAsStat = new Stat(xx);
             this.array = this.arrayAsStat.array();
             this.initialize();
        }

        public ProbabilityPlotiNICU(short[] xx){
             this.arrayAsStat = new Stat(xx);
             this.array = this.arrayAsStat.array();
             this.initialize();
        }
        ...
        public ProbabilityPlotiNICU(byte[] xx){
             this.arrayAsStat = new Stat(xx);
             this.array = this.arrayAsStat.array();
             this.initialize();
        }
        ...
        public ProbabilityPlotiNICU(BigDecimal[] xx){
             this.arrayAsStat = new Stat(xx);
             this.initialize();
        }

        public ProbabilityPlotiNICU(BigInteger[] xx){
             this.arrayAsStat = new Stat(xx);
             this.array = this.arrayAsStat.array();
             this.initialize();
        }

        public ProbabilityPlotiNICU(Object[] xx){
             this.arrayAsStat = new Stat(xx);
             this.array = this.arrayAsStat.array();
             this.initialize();
        }

        public ProbabilityPlotiNICU(Vector<Object> xx){
             this.arrayAsStat = new Stat(xx);
             this.array = this.arrayAsStat.array();
             this.initialize();
        }

        public ProbabilityPlotiNICU(ArrayList<Object> xx){
             this.arrayAsStat = new Stat(xx);
             this.array = this.arrayAsStat.array();
             this.initialize();
        }

        public ProbabilityPlotiNICU(ArrayMaths xx){
        	
            this.array = xx.array();
            this.arrayAsStat = new Stat(this.array);
            this.initialize();
       }
        public ProbabilityPlotiNICU(Stat xx){
             this.arrayAsStat = xx;
             this.array = this.arrayAsStat.array();
             this.initialize();
        }
        ...
        private void initialize(){
            ...
            ArrayMaths sortedArray = arrayAsStat.sort();
            Stat sorted = new Stat(sortedArray.array());
		...
		public void logNormalProbabilityPlot() {
	        this.lastMethod = 11;
	        ...
	        // Check for suffient data points
	        //this.logNormalNumberOfParameters = 2;
	        ...
	        Regression min = new Regression(this.sortedData, this.sortedData);
	        double meanest = this.mean;
            if(mean==0.0)meanest = this.standardDeviation/3.0;
            double[] start = {meanest, this.standardDeviation};
            this.initialEstimates = start;
            double[] step = {0.3*meanest, 0.3*this.standardDeviation};
            double tolerance = 1e-10;
            ...
            // Create an instance of ExponentialProbPlotFunc
            LogNormallProbPlotFunc lnpf = new LogNormallProbPlotFunc();
            lnpf.setDataArray(this.numberOfDataPoints);

            // Obtain best probability plot varying mu and sigma
            // by minimizing the sum of squares of the differences between the ordered data and the ordered statistic medians
            min.simplex(lnpf, start, step, tolerance);  
            
            this.logNormalParam = min.getBestEstimates();
            // Get mu and sigma value errors
            this.logNormalParamErrors = min.getBestEstimatesErrors(); 
            
            
         // Get sum of squares
            this.logNormalSumOfSquares = min.getSumOfSquares();

            // Calculate Log Normal order statistic medians (Weibull with gamma = 1)
            this.logNormalOrderMedians = Stat.logNormalOrderStatisticMedians(this.logNormalParam[0], this.logNormalParam[1], 1.0, this.numberOfDataPoints);

            // Regression of the ordered data on the Exponential order statistic medians
            Regression reg = new Regression(this.logNormalOrderMedians, this.sortedData);
            reg.linear();           
            
         // Intercept and gradient of best fit straight line
            this.logNormalLine = reg.getBestEstimates();

            // Estimated erors of the intercept and gradient of best fit straight line
            this.logNormalLineErrors = reg.getBestEstimatesErrors();

            // Correlation coefficient
            this.logNormalCorrCoeff = reg.getSampleR();    
            

            this.logNormalDone = true;
            this.probPlotDone = true;           
            
		}
		// Return logNormal mu
		public double logNormalMu(){
		    if(!this.logNormalDone)throw new IllegalArgumentException("Log Normal Probability Plot method has not been called");
		    return this.logNormalParam[0];
		}

		// Return logNormal mu error
		public double logNormalError(){
		    if(!this.logNormalDone)throw new IllegalArgumentException("Log Normal Probability Plot method has not been called");
		    return this.logNormalParamErrors[0];
		}

		// Return logNormal beta
		public double logNormalBeta(){
		    if(!this.logNormalDone)throw new IllegalArgumentException("Log Normal Probability Plot method has not been called");
		    return this.logNormalParam[1];
		}

		// Return logNormal beta error
		public double logNormalBetaError(){
		    if(!this.logNormalDone)throw new IllegalArgumentException("Log Normal Probability Plot method has not been called");
		    return this.logNormalParamErrors[1];
		}

		// Return logNormal order statistic medians
		public double[] logNormalOrderStatisticMedians(){
		    if(!this.logisticDone)throw new IllegalArgumentException("Log Normal Probability Plot method has not been called");
		    return this.logNormalOrderMedians;
		}

		// Return the logNormal gradient
		public double logNormalGradient(){
		    if(!this.logNormalDone)throw new IllegalArgumentException("Log Normal Probability Plot method has not been called");
		    return this.logNormalLine[1];
		}

		// Return the error of the logNormal gradient
		public double logNormalGradientError(){
		    if(!this.logNormalDone)throw new IllegalArgumentException("Log Normal Probability Plot method has not been called");
		    return this.logisticLineErrors[1];
		}

		// Return the logNormal intercept
		public double logNormalIntercept(){
		    if(!this.logNormalDone)throw new IllegalArgumentException("Log Normal Probability Plot method has not been called");
		    return this.logisticLine[0];
		}

		// Return the error of the logNormal intercept
		public double logNormalInterceptError(){
		    if(!this.logisticDone)throw new IllegalArgumentException("Log Normal Probability Plot method has not been called");
		    return this.logNormalLineErrors[0];
		}

		// Return the logNormal correlation coefficient
		public double logNormalCorrelationCoefficient(){
		    if(!this.logNormalDone)throw new IllegalArgumentException("LogNormal Probability Plot method has not been called");
		    return this.logNormalCorrCoeff;
		}

		// Return the sum of squares at the logNormal minimum
		public double logNormalSumOfSquares(){
		    if(!this.logNormalDone)throw new IllegalArgumentException("Log Normal Probability Plot method has not been called");
		    return this.logNormalSumOfSquares;
		}
}



//Log Normal Probabilty plot function
//Class to evaluate the two parameter log-normal function y = (yscale/x.sigma.sqrt(2.pi)).exp(-0.5[(log(x) - mu)/sd]^2).
class LogNormallProbPlotFunc implements RegressionFunction{
