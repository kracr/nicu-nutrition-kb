package com.inicu.device.nellcor;
...
public class N560 implements DeviceParser {
	private List<String> listdata;
	private String uhid;
	private String beddeviceid;
	private List<String> serverTimeList;

	public N560(List<String> listdata, List<String> serverTimeList, String uhid, String beddeviceid) {
		super();
	...
	public void parse() {
		if (!BasicUtils.isEmpty(listdata) && !BasicUtils.isEmpty(serverTimeList)) {
			...
			System.out.println("inside nellcor case working");
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		// nellcor N560 parameter are converted into generic parameter
		...
		if (!BasicUtils.isEmpty(mean)) {
			for (String key : mean.keySet()) {
				if (DeviceConstants.nellcorParamMapper.containsKey(key)) {
					mappedMean.put(DeviceConstants.nellcorParamMapper.get(key), mean.get(key));
				}
			}
		}
		return mappedMean;
