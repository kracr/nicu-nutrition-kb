package com.inicu.device.draeger;
...
/*
 * designed for entering data into kdah.hosptial
 * Can be used in addition to already running
 * infinity gateway or any other centeral monitoring.
 */
public class Babylog8000KDAH implements DeviceParser{
	...
	public Babylog8000KDAH(List<String> listdata, List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		//babylog parameter are converted into generic parameter
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.babylog8000parameter.containsKey(key)){
					mappedMean.put(DeviceConstants.babylog8000parameter.get(key), mean.get(key));					
				}
			}
		}
		return mappedMean;
	...
	public void pushDataToPostgress(HashMap<String, Float> mappedMean,
		...
		if(!BasicUtils.isEmpty(deviceService)){
			deviceService.saveDeviceVentDetailKdah(mappedMean,uhid,beddeviceid,deviceTime);
		}
	}
