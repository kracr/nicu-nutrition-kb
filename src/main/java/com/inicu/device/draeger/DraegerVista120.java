...
public class DraegerVista120 implements DeviceParser{
	...
	public DraegerVista120(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.draegerVista120ParamMapper.containsKey(key)){
					mappedMean.put(DeviceConstants.draegerVista120ParamMapper.get(key), mean.get(key));					
				}
			}
		}
		return mappedMean;
