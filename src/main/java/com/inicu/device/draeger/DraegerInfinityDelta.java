...
public class DraegerInfinityDelta implements DeviceParser{
	...
	public DraegerInfinityDelta(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.draegerInfinityDeltaParamMapper.containsKey(key)){
					mappedMean.put(DeviceConstants.draegerInfinityDeltaParamMapper.get(key), mean.get(key));					
				}
			}
		}
		return mappedMean;
