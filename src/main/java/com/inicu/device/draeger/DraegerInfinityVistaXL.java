...
public class DraegerInfinityVistaXL implements DeviceParser{
	...
	public DraegerInfinityVistaXL(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.draegerInfinityVistaXLParamMapper.containsKey(key)){
					mappedMean.put(DeviceConstants.draegerInfinityVistaXLParamMapper.get(key), mean.get(key));					
				}
			}
		}
		return mappedMean;
