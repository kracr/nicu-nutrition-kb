...
public class DraegerInfinityVista implements DeviceParser{
	...
	public DraegerInfinityVista(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.draegerInfinityVistaParamMapper.containsKey(key)){
					mappedMean.put(DeviceConstants.draegerInfinityVistaParamMapper.get(key), mean.get(key));					
				}
			}
		}
		return mappedMean;
