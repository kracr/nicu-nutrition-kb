...
public class EvitaXl implements DeviceParser {
	...
	public EvitaXl(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		//evita xl parameter are converted into generic parameter
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.draegerEvitaParamMapper.containsKey(key)){
					mappedMean.put(DeviceConstants.draegerEvitaParamMapper.get(key), mean.get(key));					
				}
			}
		}
		return mappedMean;
