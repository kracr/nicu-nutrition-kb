...
public class DraegerVN500 implements DeviceParser{
	...
	public DraegerVN500(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.draegerVN500parameter.containsKey(key)){
					mappedMean.put(DeviceConstants.draegerVN500parameter.get(key), mean.get(key));					
				}
			}
		}
		return mappedMean;
