...
public class DraegerIsolette8000 implements DeviceParser{
	...
	public DraegerIsolette8000(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.DRAEGERISOLETTE8000ParamMapper.containsKey(key)){
					mappedMean.put(DeviceConstants.DRAEGERISOLETTE8000ParamMapper.get(key), mean.get(key));					
				}
			}
		}
		return mappedMean;
	...
	public void pushDataToPostgress(HashMap<String, Float> mappedMean,
		...
		if(!BasicUtils.isEmpty(deviceService)){
			deviceService.saveDeviceIncubatorWarmerDetail(mappedMean,uhid,beddeviceid,deviceTime);
		}
	}
}
