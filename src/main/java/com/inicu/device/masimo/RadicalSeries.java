package com.inicu.device.masimo;
...
public class RadicalSeries implements DeviceParser{	
	private List<String> listdata;
	private String uhid;
	private String beddeviceid;
	private List<String> serverTimeList;
	
	public RadicalSeries(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.masimoRadicalParamMapper.containsKey(key)){
					mappedMean.put(DeviceConstants.masimoRadicalParamMapper.get(key), mean.get(key));					
				}
			}
		}
		return mappedMean;
