...
public class Giraffe implements DeviceParser {
	...
	public Giraffe(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.GEGIRAFFEParamMapper.containsKey(key)){
					mappedMean.put(DeviceConstants.GEGIRAFFEParamMapper.get(key), mean.get(key));
				}
			}
		}
		return mappedMean;
