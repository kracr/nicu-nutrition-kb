...
public class B40 implements DeviceParser {
	...
	public B40(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.geb40ParamMapper.containsKey(key)){
					mappedMean.put(DeviceConstants.geb40ParamMapper.get(key), mean.get(key));					
				}
			}
		}
		return mappedMean;
