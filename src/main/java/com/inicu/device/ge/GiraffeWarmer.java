...
public class GiraffeWarmer implements DeviceParser {
	...
	public GiraffeWarmer(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.GEGIRAFFEWARMERParamMapper.containsKey(key)){
					mappedMean.put(DeviceConstants.GEGIRAFFEWARMERParamMapper.get(key), mean.get(key));
				}
			}
		}
		return mappedMean;
