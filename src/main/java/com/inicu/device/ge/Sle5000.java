...
public class Sle5000 implements DeviceParser {
	...
	public Sle5000(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		//sle5000 parameter are converted into generic parameter
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.sle5000ParamMapper.containsKey(key.trim())){
					try{
						if(DeviceConstants.sle5000ParamMapper.get(key.trim()).equalsIgnoreCase(DeviceConstants.DEVICE_VENTILATOR_TIDALVOL)){
							mappedMean.put(DeviceConstants.sle5000ParamMapper.get(key.trim()), (float) (mean.get(key)*0.2));																		
						}else if(DeviceConstants.sle5000ParamMapper.get(key.trim()).equalsIgnoreCase(DeviceConstants.DEVICE_VENTILATOR_TI)){
							mappedMean.put(DeviceConstants.sle5000ParamMapper.get(key.trim()), (float) (mean.get(key)*0.01));
						}else{
							mappedMean.put(DeviceConstants.sle5000ParamMapper.get(key.trim()), mean.get(key));
						}
					}catch(Exception e){
						e.printStackTrace();
					}
	...
	public void pushDataToPostgress(HashMap<String, Float> mappedMean,
		...
		if(!BasicUtils.isEmpty(deviceService)){
			deviceService.saveVentilatorDetail(mappedMean,uhid,beddeviceid,deviceTime);
		}
	}
