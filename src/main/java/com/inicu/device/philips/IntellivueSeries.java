...
public class IntellivueSeries implements DeviceParser{
	...
	public IntellivueSeries(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.philipsIntellivueParamMapper.containsKey(key)){
					mappedMean.put(DeviceConstants.philipsIntellivueParamMapper.get(key), mean.get(key));					
				}
			}
		}
		return mappedMean;
