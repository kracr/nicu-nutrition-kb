...
public class SureSign implements DeviceParser{
	...
	public SureSign(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.philipsSureSignParamMapper.containsKey(key)){
					mappedMean.put(DeviceConstants.philipsSureSignParamMapper.get(key), mean.get(key));					
				}
			}
		}
		return mappedMean;
