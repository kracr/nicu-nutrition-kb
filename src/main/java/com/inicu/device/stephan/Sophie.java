package com.inicu.device.stephan;
...
public class Sophie implements DeviceParser {
	...
	public Sophie(List<String> listdata, List<String> serverTimeList, String uhid, String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		// sophie ventilator parameter are converted into generic parameter
		...
		if (!BasicUtils.isEmpty(mean)) {
			for (String key : mean.keySet()) {
				if (DeviceConstants.sophieparameter.containsKey(key)) {
					mappedMean.put(DeviceConstants.sophieparameter.get(key), mean.get(key));
				}
			}
		}
		return mappedMean;
