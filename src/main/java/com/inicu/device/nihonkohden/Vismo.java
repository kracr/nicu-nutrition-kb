package com.inicu.device.nihonkohden;
...
public class Vismo implements DeviceParser{
	...
	public Vismo(List<String> listdata,List<String> serverTimeList,String uhid,String beddeviceid) {
		super();
	...
	public HashMap<String, Float> parameterMapper(HashMap<String, Float> mean) {
		...
		//vismo parameter are converted into generic parameter
		...
		if(!BasicUtils.isEmpty(mean)){
			for(String key:mean.keySet()){
				if(DeviceConstants.vismoParamMapper.containsKey(key)){
					mappedMean.put(DeviceConstants.vismoParamMapper.get(key), mean.get(key));					
				}
			}
		}
		return mappedMean;
